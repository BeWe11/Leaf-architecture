...
class Cycle():
    ...
    def ordered_edges(self, edges):
        ...
        for comp in con:
            ...
            # coordinates of path
            coords = array([(G.node[u]['pos'][0], G.node[u]['pos'][1])
                    for u, v in sorted_edges] \
                            + [(G.node[sorted_edges[0][0]]['pos'][0],
                                G.node[sorted_edges[0][0]]['pos'][1])])
...
def traverse_graph(G, start, nextn):
    ...
    while cur != start:
        ...
        if n_neighs > 1:
            ...
            prev_coords = array([G.node[prev]['pos'][0], G.node[prev]['pos'][1]])
            neigh_coords = array([[G.node[n]['pos'][0], G.node[n]['pos'][1]] \
                for n in neighs])
